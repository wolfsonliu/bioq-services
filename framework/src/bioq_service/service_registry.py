"""Service registry — load the repo-root `services.yaml` (svc -> ServiceRecord).
...
def find_services_yaml(start: Path | None = None) -> Path:
    """Walk up from `start` (or cwd) until the repo-root `services.yaml` is found.

    Raises FileNotFoundError if not found — typically means the caller is
    running from outside the bioq-services repo.
    """
    base = (start or Path.cwd()).resolve()
    for candidate in [base, *base.parents]:
        target = candidate / "services.yaml"
        if target.is_file():
            return target
    raise FileNotFoundError(f"services.yaml not found above {base!r}")
