...
job 生命周期 / 错误处理 / 持久化 / manifest / CLI / 上传下载这些通用层。

本页是新增一个服务的单一权威流程文档；若与子页、`services/<svc>-server/README.md` 冲突，以本 cookbook 为准。
...
存放位置统一为 [`docs/specs/`](../specs/)（与既有 `*-design.md` 并列）。
...
| 子页 | 内容 |
|------|------|
| [skeleton](./skeleton.zh.md) | 5 分钟 echo skeleton：`__init__` / `settings` / `models` / `adapter` / `app.py`（含 task endpoint）/ `__main__` / `pyproject` / `VERSION` / `README` |
| [dockerfile](./dockerfile.zh.md) | Dockerfile：`vendor.sh` / `fetch_weights.sh` / uv + conda 骨架 / wrapper vs patch 决策 |
| [conda-pitfalls](./conda-pitfalls.zh.md) | 包装 conda-based upstream 的常见陷阱（LANG / yaml override / dead-import stub / uv git+ 等） |
| [testing](./testing.zh.md) | `test_app` / `test_cli` / `test_fc` / `test_fc_task` 测试骨架 |
| [deploy](./deploy.zh.md) | 部署到 FC + 控制台配置（异步任务模式 + OSS mount）+ 并发能力测试 |

## 0. 先写设计文档（**开工前必做**）

新 service 落地**先出一份 `YYYY-MM-DD-<svc>-server-design.md` 设计文档**（归档到
[`docs/specs/`](../specs/)），然后再动代码。这不是仪式感——本 guide 是"HOW 起骨架"，设计文档是
"WHY 这么设计 + 与项目其它 service 的边界"，两者互补缺一不可。

**设计文档写作规范**：H1 标题 + 元数据行（日期 / 状态 / 适用 / 相关）+ 下表的必备章节；写完后在
`docs/specs/` 里加一行链接。
...
| 章节 | 关键内容 |
|---|---|
| 概述 | 上游模型/算法定位、在本项目 pipeline 中扮演什么角色、与已有 service 的边界 |
...
写完设计文档并同步 `docs/specs/` 后，再回到本 guide 从"必备文件清单"起做实现。
...
| `uris.py` | URI scheme 解析（job:// / oss:// / file:// / http(s):// 等）——**现由框架统一提供** | [framework uris.py](../../framework/src/bioq_service/uris.py) |
...
- [ ] **设计文档已就位**：`YYYY-MM-DD-<svc>-server-design.md` 含 §0 列出的必备章节，且已归档到 [docs/specs/](../specs/)
...
- [ ] **经 gateway 调用的服务**：FC 控制台把数据面 OSS bucket `bioagent-inputs` 挂到 `/mnt/oss`（RW）；Dockerfile runtime 有 `ENV <SVC>_OSS_OUTPUT_MOUNT=/mnt/oss`；framework 用 `COPY framework`（非 bind-mount，否则 output-sink 修复进不去镜像）；有文件输入的服务其 `uris.py` 必须支持裸 `/` 绝对路径（gateway 把 `oss://` 改写成 `/mnt/oss/...`，下游用 `shutil.copy2` 直读）
- [ ] FC 部署后 `curl /healthz/detail` 验证 `weights_loaded: true`
- [ ] Dockerfile 用 uv venv 骨架 或 conda/micromamba 多阶段构建
- [ ] **conda 服务**：Dockerfile runtime 阶段有 `ENV LANG=C.UTF-8` + `ENV LC_ALL=C.UTF-8`（关闭 upstream `open()` ASCII locale 陷阱，见 [conda-pitfalls.zh.md](./conda-pitfalls.zh.md)）
...
- 框架 API / 约定：[framework-api.zh.md](../topics/framework-api.zh.md) · [conventions.zh.md](../topics/conventions.zh.md)
- 构建 / 验证：[build-deploy.zh.md](../topics/build-deploy.zh.md) · [testing.zh.md](../topics/testing.zh.md)
- 经 gateway 调用：[gateway.zh.md](../topics/gateway.zh.md) · [service-anatomy.zh.md](../topics/service-anatomy.zh.md)
- 设计文档归档：[docs/specs/](../specs/)
- 参考实现：
