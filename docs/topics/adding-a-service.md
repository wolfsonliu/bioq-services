...
> **Source**: points at the existing authoritative docs (currently Chinese) — [`../adding-a-service.zh.md`](../adding-a-service.zh.md) (repo-local landing) and [`../adding-a-new-service/index.zh.md`](../adding-a-new-service/index.zh.md) (cookbook).
> **Refresh/remove when**: those authoritative docs move or the division between them changes.
...
2. **Build the skeleton** from the cookbook sub-pages ([`skeleton`](../adding-a-new-service/skeleton.zh.md),
   [`dockerfile`](../adding-a-new-service/dockerfile.zh.md), [`conda-pitfalls`](../adding-a-new-service/conda-pitfalls.zh.md),
   [`testing`](../adding-a-new-service/testing.zh.md), [`deploy`](../adding-a-new-service/deploy.zh.md)); the
   repo-local steps (naming / required files / pyproject / Dockerfile conventions / registration /
   checklist) are in [`../adding-a-service.zh.md`](../adding-a-service.zh.md).
...
- Add `services/<svc>-server/upstream/` and `services/<svc>-server/weights/` to `.gitignore`.
