"""FC async task mode tests for flowmol-server (opt-in).

Run with::

    RUN_FC_TESTS=1 \\
    uv run python -m pytest -m fc services/flowmol-server/tests/test_fc_task.py -v
...
FlowMol generation is unconditional — no file uploads — so all payloads
sit comfortably under FC's 128 KiB async event cap.  No sync-bootstrap
staging needed.
"""
...
SERVICE = "flowmol-server"

# n_mols=10 / n_timesteps=100 on T4: ~10-30 s; cold start + NAS load ~2 min.
# Give a generous ceiling.
POLL_TIMEOUT_S = 900
POLL_INTERVAL_S = 15
...
@pytest.fixture(scope="module")
def dedup_task_id() -> str:
    return f"fc-async-dedup-{int(time.time())}-{uuid.uuid4().hex[:6]}"
...
def _get_with_retry(
    ...
    *,
    ...
    After long-running async tasks the FC HTTP gateway can rate-limit
    subsequent GETs — a platform-layer artifact, not a server-side bug.
...
class TestAsyncGenerate:
    ...
    def test_input_params_echoed(self, generate_task):
        ...
        assert params.get("n_mols") == 10
        assert params.get("n_timesteps") == 100
        assert params.get("model_variant") == "flowmol3"
        assert params.get("seed") == 42

    def test_molecules_sdf_downloadable(
        self, client, generate_task_id, generate_task,
    ):
        r = _get_with_retry(
            client, f"/api/jobs/{generate_task_id}/file/molecules.sdf"
        )
        assert r.status_code == 200
    ...
    def test_sampling_stats_json(self, client, generate_task_id, generate_task):
        r = _get_with_retry(
            client, f"/api/jobs/{generate_task_id}/file/sampling_stats.json"
        )
        assert r.status_code == 200
        stats = r.json()
        assert stats["n_requested"] == 10
        assert stats["n_written"] >= 1
        assert stats["n_timesteps"] == 100
...
class TestJobLifecycle:
    ...
    def test_download_zip(self, client, generate_task_id, generate_task):
        ...
        assert any("molecules.sdf" in n for n in names), (
            f"molecules.sdf missing from zip: {names}"
        )
...
class TestAsyncDuplicateDedup:
    ...
    def test_duplicate_does_not_rerun(
        ...
        r2 = client.post(
            "/api/tasks/generate",
            data={"n_mols": "10", "n_timesteps": "100", "seed": "999"},
            headers=_async_headers(generate_task_id),
        )
