"""Input URI resolution for diffusion-hopping-server.

Mirrors the boltzgen / deeprank-ab pattern: upload / job:// / file:// /
oss:// / http(s)://.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: DiffusionHoppingSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: DiffusionHoppingSettings) -> Path:
    try:
