"""Input URI resolution for boltzgen-server.

Mirrors the deeprank-ab-server / proteinmpnn-server pattern: upload / job:// /
file:// / oss:// / http(s)://.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: BoltzGenSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: BoltzGenSettings) -> Path:
    try:
