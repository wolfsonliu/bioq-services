"""Input URI resolution for pocketxmol-server.

Standard 5-scheme resolver (upload / job:// / file:// / oss:// / http(s)://)
mirrored across boltzgen / diffusion-hopping / rfantibody / drughive.

All input kinds (protein PDB, ligand SDF/PDB, reference ligand, peptide
PDB) go through the same helper.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: PocketXMolSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: PocketXMolSettings) -> Path:
    try:
