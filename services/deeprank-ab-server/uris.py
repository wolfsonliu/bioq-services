"""Input URI resolution for deeprank-ab-server.

Mirrors the dockq-server / proteinmpnn-server pattern: upload / job:// /
file:// / oss:// / http(s)://.
"""
...
def resolve_input(
    ...
    raise HTTPException(status_code=422, detail="Either an upload or input_pdb_uri is required.")


def _resolve_uri(uri: str, dest: Path, settings: DeepRankAbSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: DeepRankAbSettings) -> Path:
    try:
