"""Input URI resolution for iggm-server.

Mirrors the boltzgen-server / deeprank-ab-server pattern: upload / job:// /
file:// / oss:// / http(s)://.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: IgGMSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: IgGMSettings) -> Path:
    try:
