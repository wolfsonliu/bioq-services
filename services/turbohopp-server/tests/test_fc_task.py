"""FC async task mode tests for turbohopp-server (opt-in).

Run with::

    RUN_FC_TESTS=1 \\
    uv run python -m pytest -m fc services/turbohopp-server/tests/test_fc_task.py -v
...
``/data/turbohopp_jobs/<bootstrap_id>/input/{protein.pdb,reference_ligand.sdf}``
as a side effect of ``JobRunner.submit``.  Async tests then reference both
via ``file://`` URIs.  Net cost: 1 extra inference run.

Override the bootstrap with ``TURBOHOPP_TEST_PROTEIN_NAS_PATH=`` /
``TURBOHOPP_TEST_LIGAND_NAS_PATH=`` env vars if pre-staged elsewhere on NAS.
"""
...
PRESTAGED_PROTEIN = os.environ.get("TURBOHOPP_TEST_PROTEIN_NAS_PATH")
PRESTAGED_LIGAND = os.environ.get("TURBOHOPP_TEST_LIGAND_NAS_PATH")

# Jobs base dir on the FC instance — must match Dockerfile's
# TURBOHOPP_JOBS_BASE_DIR (settings default).
JOBS_BASE_DIR_ON_FC = "/data/turbohopp_jobs"
...
def staged_uris(client: httpx.Client) -> tuple[str, str]:
    ...
    The sync POST runs a real (short) generation in the background — we
    don't need its output, only the side-effect of saving both inputs to
    NAS before ``submit`` returns.
    """
...
def _poll_to_completion(client: httpx.Client, task_id: str) -> dict:
    ...
    # Effective retry buffer: 60 × POLL_INTERVAL_S seconds of 429s.
    final = poll_job(
        client, "", task_id,
...
def _assert_completed_with_sdf(
    ...
    *,
    ...
    """Assert a completed task produced at least one .sdf file."""
...
class TestAsyncGenerate:
    def test_completed(self, generate_task, generate_task_id, client):
        # Consistency sampling at 5 steps is fast (~15-60s on H20); keep
        # the min-duration low.  If duration is < 3s, something's wrong.
        _assert_completed_with_sdf(
            generate_task, generate_task_id, client, min_duration_s=3.0,
        )
