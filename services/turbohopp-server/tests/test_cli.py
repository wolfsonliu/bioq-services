"""CLI batch-mode tests for turbohopp-server."""
...
from server.adapter import TurboHoppAdapter
...
def test_generate_build_argv(tmp_path):
    ...
    (tmp_path / "weights" / "ckpt.ckpt").write_bytes(b"\x00")
    job_dir = tmp_path / "j"
    job_dir.mkdir()
    ...
    argv = _generate_build(
        GenerateRequest(num_samples=5, num_sampling_steps=8),
    ...
    assert "--num_sampling_steps" in argv and "8" in argv
    assert "--find_best" not in argv  # default False → omitted
...
def test_cli_generate_success(tmp_path):
    ...
    adapter = TurboHoppAdapter(settings=s)
