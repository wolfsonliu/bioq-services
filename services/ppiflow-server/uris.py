"""Input URI resolution shared across all ppiflow-server endpoints.

Mirrors the scheme set used by rfantibody-server so an agent learns one
convention across services: upload / job:// / file:// / oss:// / http(s)://.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: PPIFlowSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: PPIFlowSettings) -> Path:
    try:
