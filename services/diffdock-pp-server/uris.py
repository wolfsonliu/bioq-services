"""Input URI resolution for diffdock-pp-server.

Mirrors the diffusion-hopping / boltzgen / deeprank-ab pattern: upload /
job:// / file:// / oss:// / http(s)://.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: DiffDockPPSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: DiffDockPPSettings) -> Path:
    try:
