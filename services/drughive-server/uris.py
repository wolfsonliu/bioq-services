"""Input URI resolution for drughive-server.

Standard 5-scheme resolver (upload / job:// / file:// / oss:// / http(s)://)
mirrored across boltzgen / diffusion-hopping / rfantibody.  All three input
kinds — target PDB, reference ligand SDF, optional substruct fragment SDF,
optional PDBQT — go through the same helper.
"""
...
def maybe_resolve_input(
    ...
    neither upload nor URI is provided — for genuinely optional inputs
    like ``substruct_modify`` (SMARTS pattern is an alternative)."""
    if input_uri is None and upload is None:
        return None
    return resolve_input(upload, input_uri, dest, settings)


def _resolve_uri(uri: str, dest: Path, settings: DrughiveSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: DrughiveSettings) -> Path:
    try:
