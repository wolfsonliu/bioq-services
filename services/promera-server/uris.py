"""Input URI resolution for promera-server.

Standard URI scheme: upload / job:// / file:// / oss:// / http(s)://.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: PromeraSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: PromeraSettings) -> Path:
    try:
