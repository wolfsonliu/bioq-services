...
# FC HeaderField session affinity name.  Must match
#   (a) the Dockerfile ENV `<SERVICE>_SESSION_HEADER_NAME=bioagent-session-id`
#       used by every deployed bioagent GPU service, and
#   (b) the FC console's "HeaderField" affinity setting on each function.
# In HeaderField mode FC routes by the header *value* in the incoming
# request (client-supplied; not generated by FC), so we use the stable
# `task_id` as the value on every request — submit / poll / fetch — and
# FC binds whichever instance handles the first match to the session.
SESSION_AFFINITY_HEADER = "bioagent-session-id"
...
class HTTPDispatcher:
    ...
    def submit(
        ...
        *,
        ...
        headers = {
            ...
            # Session affinity from request 1.  FC binds the chosen worker
            # instance to this session value so subsequent poll/fetch
            # requests carrying the same value route back to it (instead
            # of waking a fresh instance to read NAS).
            SESSION_AFFINITY_HEADER: task_id,
        }
    ...
    def get_status(self, handle: DispatchHandle) -> TaskStatus:
        ...
        Carries the session-affinity header so FC routes the poll back to the
        instance that ran the task instead of cold-starting a new one — every
        poll otherwise wakes a fresh instance (no JobInfo cached) just to read
        NAS, which inflates instance count + cost.  Same pattern as
        ``fetch_result``.
