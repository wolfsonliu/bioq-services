...
Same three-scheme contract as rfdiffusion-server (upload / file:// / job:// /
oss:// / http(s)://). Each service ships its own copy so URI schemes can
evolve independently without touching the shared framework.

RFdiffusion2 only ever ingests one input file (the motif/ligand PDB), so we
ship one combined helper (`resolve_input`).
"""
...
def resolve_input(
    ...
    """Land a file at `dest` from either an upload or a URI. HTTP 422 if neither."""
    if input_uri:
...
def _resolve_uri(uri: str, dest: Path, settings: RFdiffusion2Settings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
    ...
    if uri.startswith("job://"):
        ...
        # Pull from the previous job's output dir on the shared NAS.
...
def _download_from_oss(uri: str, dest: Path, settings: RFdiffusion2Settings) -> Path:
    try:
