"""Input URI resolution for odesign-server.

Mirrors the boltzgen-server pattern: upload / job:// / file:// / oss:// / http(s)://.
Adds JSON ref_file rewriting for ODesign's input format.
"""
...
def resolve_input(
    ...
    raise HTTPException(status_code=422, detail="Either an upload or URI is required.")
...
def _resolve_uri(uri: str, dest: Path, settings: ODesignSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: ODesignSettings) -> Path:
    try:
