...
The same helpers cover three useful patterns:

  * **Upload**: client multiparts a file → save to `<job_dir>/input/<name>`
  * **Local NAS path** (most efficient for pipelines on shared NAS):
    `file:///data/rfantibody_jobs/<id>/output/1_rfdiffusion.qv` or just the path
  * **Cross-job reference**: `job://<id>/<filename>` — pulls from a previous
    job's output dir on the same NAS
  * **Remote** (cold-start clients without NAS access): `oss://` or `http(s)://`

All branches end up copying the bytes into the new job's `<job_dir>/input/...`,
keeping each job's inputs self-contained.
"""
...
def resolve_input(
    ...
    """Land a file at `dest` from either an upload or a URI. Raises HTTP 422 if neither."""
    if input_uri:
    ...
    raise HTTPException(
        status_code=422,
        detail="Either a file upload or `input_uri` is required",
    )


def _resolve_uri(uri: str, dest: Path, settings: RFantibodySettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_from_oss(uri: str, dest: Path, settings: RFantibodySettings) -> Path:
    try:
