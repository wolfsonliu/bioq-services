"""Input URI resolution for diffdock-server.

Standard 5-scheme resolver (upload / job:// / file:// / oss:// / http(s)://)
mirrored across boltz / diffdock-pp / drughive.  Both protein PDB and ligand
SDF go through the same helper; the ligand branch also accepts SMILES
strings directly (bypasses resolver, passed as a CLI --ligand_description).
"""
...
def maybe_resolve_input(
    ...
    neither upload nor URI is provided — for inputs that have an
    alternative representation (protein_sequence for the protein input,
    ligand_description SMILES for the ligand input)."""
    if input_uri is None and upload is None:
        return None
    return resolve_input(upload, input_uri, dest, settings)


def _resolve_uri(uri: str, dest: Path, settings: DiffdockSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: DiffdockSettings) -> Path:
    try:
