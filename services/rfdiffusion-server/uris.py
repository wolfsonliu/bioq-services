...
Same three-scheme contract as rfantibody-server (upload / file:// / job://
/ oss:// / http(s)://) — kept here as a separate module so each service can
evolve its URI schemes independently without churning the shared framework.

For RFdiffusion the only file ever flowing in is the input PDB, so we ship one
combined helper (`resolve_input`) that lets the client pick whichever delivery
mechanism is cheapest for them.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: RFdiffusionSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
    ...
    if uri.startswith("job://"):
        ...
        # Pull from the previous job's output directory on the shared NAS.
...
def _download_from_oss(uri: str, dest: Path, settings: RFdiffusionSettings) -> Path:
    try:
