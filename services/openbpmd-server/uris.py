"""Input URI resolution for openbpmd-server.

Mirrors the diffdock-pp / diffusion-hopping pattern: upload / job:// /
file:// / oss:// / http(s)://.
"""
...
def resolve_input(
    ...
    *,
    ...
    raise HTTPException(
        status_code=422,
        detail=f"{field_name}: either an upload or a URI is required.",
    )


def _resolve_uri(uri: str, dest: Path, settings: OpenBPMDSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: OpenBPMDSettings) -> Path:
    try:
