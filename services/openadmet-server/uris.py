"""Resolve input file references (UploadFile or URI string) to a local path.

Same pattern as rfantibody-server/uris.py — covers uploads, cross-job
`job://` refs, local NAS paths (`file://` / absolute), OSS, and http(s).
"""
...
def resolve_input(
    ...
    """Land a file at `dest` from either an upload or a URI. Raises 422 if neither."""
    if input_uri:
...
def _resolve_uri(uri: str, dest: Path, settings: OpenAdmetSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_from_oss(uri: str, dest: Path, settings: OpenAdmetSettings) -> Path:
    try:
