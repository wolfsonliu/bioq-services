"""Input URI resolution for dockq-server.

Mirrors proteinmpnn-server / rfantibody-server schemes so agents learn one set
of conventions: upload / job:// / file:// / oss:// / http(s)://. Used by both
the single-file native/model upload and the batch driver.
"""
...
def _resolve_uri(uri: str, dest: Path, settings: DockQSettings) -> Path:
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: DockQSettings) -> Path:
    try:
