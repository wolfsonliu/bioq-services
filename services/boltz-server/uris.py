"""Input URI resolution for boltz-server.

Mirrors dockq-server / rfantibody-server schemes so agents learn one set of
conventions: upload / job:// / file:// / oss:// / http(s)://.

Used for:
  * MSA files (per-chain .a3m referenced via SequenceEntry.msa_uri)
  * Template files (CIF or PDB referenced via TemplateEntry.cif_uri / pdb_uri)
  * The raw YAML escape hatch (_BoltzCommon.raw_yaml_uri)
"""
...
def resolve_uri(uri: str, dest: Path, settings: BoltzSettings) -> Path:
    """Resolve a single URI to a local file at `dest`. Idempotent — caller picks the path."""
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _download_oss(uri: str, dest: Path, settings: BoltzSettings) -> Path:
    try:
