...
def test_result_url_if_exists_hit_and_miss():
    ...
    assert p.result_url_if_exists("alice", "job1", "results.zip") == "https://oss.get/signed"
    client.head_object.side_effect = _oss_not_found()
    assert p.result_url_if_exists("alice", "job1", "results.zip") is None
...
def test_exists_propagates_oss_outage():
    ...
    with pytest.raises(Exception, match="AccessDenied"):
        p.prepare_upload("alice", "job1", "x.pdb")
    client.presign.assert_not_called()
